...
// BƯỚM NHIỀU MÀU
function createButterflyWingGeo() {
  const shape = new THREE.Shape();
  shape.moveTo(0, 0);
  shape.quadraticCurveTo(0.5, 0.35, 0.42, 0.05);
  shape.quadraticCurveTo(0.55, -0.25, 0.15, -0.35);
  shape.quadraticCurveTo(0.05, -0.15, 0, 0);
  const geo = new THREE.ShapeGeometry(shape);
  geo.rotateX(-Math.PI / 2);
  return geo;
}
const butterflyWingGeo = createButterflyWingGeo();

const butterflyColors = [
  0xff3d68, 0xffb703, 0x06d6a0, 0x8338ec, 0x00b4d8, 0xff006e, 0xffd60a,
];
...
function createButterfly(color) {
  ...
  const wingLeft = new THREE.Mesh(butterflyWingGeo, wingMat);
  const pivotLeft = new THREE.Group();
  pivotLeft.add(wingLeft);
  group.add(pivotLeft);

  const wingRight = new THREE.Mesh(butterflyWingGeo, wingMat);
  wingRight.scale.x = -1;
  const pivotRight = new THREE.Group();
  pivotRight.add(wingRight);
  group.add(pivotRight);

  const bodyGeo = new THREE.SphereGeometry(0.05, 6, 6);
  bodyGeo.scale(1, 1, 3.2);
  const bodyMat = new THREE.MeshStandardMaterial({ color: 0x2a1a1a });
  const bodyMesh = new THREE.Mesh(bodyGeo, bodyMat);
  group.add(bodyMesh);

  return { group, pivotLeft, pivotRight };
}

const butterflies = [];
const BUTTERFLY_COUNT = isMobile ? 5 : 8;
for (let i = 0; i < BUTTERFLY_COUNT; i++) {
  const color = butterflyColors[i % butterflyColors.length];
  const { group, pivotLeft, pivotRight } = createButterfly(color);
  ...
  butterflies.push({
    ...
    orbitSpeed: (0.15 + Math.random() * 0.2) * (i % 2 === 0 ? 1 : -1),
    phase: Math.random() * Math.PI * 2,
    baseY: 5 + Math.random() * 8,
...
function updateButterflies(time) {
  butterflies.forEach((b) => {
    ...
    const sign = Math.sign(b.orbitSpeed) || 1;

    const x = Math.cos(angle) * b.orbitRadius;
    const z = Math.sin(angle) * b.orbitRadius;
    const y = b.baseY + Math.sin(time * b.bobSpeed) * b.bobHeight;
    ...
    const flap = 0.3 + Math.abs(Math.sin(time * b.flapSpeed)) * 0.9;
    b.pivotLeft.rotation.z = flap;
    b.pivotRight.rotation.z = -flap;
  });
}
...
const wishList = [
  {
    text: "Chúc em một mùa Trung Thu thật vui và ấm áp. Mong rằng những điều tốt đẹp nhất sẽ luôn đến với em.",
    img: "./assets/1.jpg",
  },
  {
    text: "Trung Thu này có bánh, có trăng, có đèn lồng, nhưng điều anh mong nhất vẫn là được ở bên em.",
    img: "./assets/2.jpg",
  },
  {
    text: "Cảm ơn em vì đã xuất hiện và khiến những ngày bình thường của anh trở nên đặc biệt hơn.",
    img: "./assets/3.jpg",
  },
  {
    text: "Mong em luôn vui vẻ, luôn cười thật nhiều và đừng để những chuyện buồn làm em mất đi nụ cười nhé.",
    img: "./assets/4.jpg",
  },
  {
    text: "Nếu trăng đêm nay có đẹp đến đâu thì anh vẫn thấy em là điều đáng yêu nhất của mùa Trung Thu này.",
    img: "./assets/5.jpg",
  },
  {
    text: "Chúc người anh thương một mùa Trung Thu thật hạnh phúc. Hy vọng những mùa Trung Thu sau, chúng ta vẫn luôn có nhau.",
    img: "./assets/6.jpg",
  },
  {
    text: "Phương này, Trung Thu này cho anh xin một cơ hội được thích Bống nhé",
    img: "./assets/7.jpg",
  },
];
...
// BẦU TRỜI SAO NHIỀU LỚP, LUNG LINH
const starLayers = [];
const starLayerConfigs = [
  { count: isMobile ? 250 : 550, size: 0.35, color: 0xffffff, baseOpacity: 0.5, speed: 1.2 },
  { count: isMobile ? 120 : 260, size: 0.55, color: 0xfff4d6, baseOpacity: 0.65, speed: 0.8 },
  { count: isMobile ? 60 : 140, size: 0.85, color: 0xffffff, baseOpacity: 0.85, speed: 0.5 },
];

starLayerConfigs.forEach((cfg, layerIndex) => {
...
function updateStarTwinkle(time) {
  starLayers.forEach((layer) => {
    const twinkle = 0.25 * Math.sin(time * layer.speed + layer.phase);
    layer.material.opacity = Math.max(0, layer.baseOpacity + twinkle);
  });
}
...
function launchShootingStar(star) {
  ...
  star.dir.set(-1, -0.5, Math.random() * 0.3 - 0.15).normalize();
  star.sprite.position.copy(star.startPos);
  star.sprite.visible = true;
  star.active = true;
  star.life = 0;
  star.duration = 0.8 + Math.random() * 0.4;
}
...
function updateShootingStars(delta) {
  shootingStars.forEach((star) => {
    ...
    star.sprite.position.addScaledVector(star.dir, star.speed * delta);
...
function spawnTapHeart(x, y) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    el.textContent =
      tapHeartSymbols[Math.floor(Math.random() * tapHeartSymbols.length)];
    el.style.left = x + "px";
    el.style.top = y + "px";
